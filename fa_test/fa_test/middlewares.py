...
class MyUserAgentMiddleware(UserAgentMiddleware):
    """
    这是一个自定义的User-Agent中间件类，它继承自Scrapy框架中的 UserAgentMiddleware 类。通过继承，我们可以覆盖或扩展原始的User-Agent中间件的行为，以满足我们的需求。

def process_request(self, request, spider):是 MyUserAgentMiddleware 类的一个方法，它在处理每个请求之前被调用。在Scrapy中，中间件的 process_request 方法可以用于在请求发送前对请求进行预处理。
