...
class UnittestSpider(scrapy.Spider):
            ...
            if type_tuple:
                yield scrapy.Request(
                    ...
                    meta={
                        ...
                        'origin_url_index': origin_url_index,
                        'origin_url': origin_url  # 添加这一行，将 origin_url 作为 meta 参数传递给回调函数
                    },
                    dont_filter=True
                )
    ...
    def parse(self, response, **kwargs):
        ...
        product_list = re.findall(r'window.boostSDFallback = ([\s\S]*)}].', response.text)
    ...
    # pprint.pprint(response.text) 可以先打印下响应数据查看有是否我们要定位的内容   (product_list)
    ...
    def parse_details(self, response: HtmlResponse):
        ...
        item["product_title"] = ' '.join(product_title.split(" ")[1:]) # 去掉产品标题的第一个单词
